...
@DisplayName("P0: 组装单业务校验")
public class P0AssembleValidationTest extends ApiTestBase {

    private static final List<JSONObject> materials = new ArrayList<>();
    private static Long depotId;

    @BeforeAll
    static void prepareAssembleData() {
        P0AssembleValidationTest test = new P0AssembleValidationTest();
    ...
    @Test
    @DisplayName("服务端按子件成本生成组合件并支持审核回滚")
    void normalizeCostAndAudit() {
        assumeTrue(hasBaseData(), "缺少组装测试基础数据");
        String number = generateNumber("ZZD");
        Long headId = null;
        boolean audited = false;
        try {
            Response response = submitAssemble(number, "0", depotId, normalRows(false));
    ...
    @Test
    @DisplayName("拒绝非法状态、明细结构、同物料、越权仓库和库存不足")
    void rejectInvalidAssembly() {
        assumeTrue(hasBaseData(), "缺少组装测试基础数据");
        assertBizCode(submitAssemble(generateNumber("ZZD"), "3", depotId, normalRows(false)),
                ExceptionConstants.DEPOT_HEAD_ASSEMBLE_STATUS_CODE);

        JSONArray invalidStructure = normalRows(false);
        invalidStructure.getJSONObject(2).put("mType", "非法类型");
        assertBizCode(submitAssemble(generateNumber("ZZD"), "0", depotId, invalidStructure),
                ExceptionConstants.DEPOT_HEAD_ASSEMBLE_STRUCTURE_CODE);

        JSONArray sameMaterial = normalRows(true);
        assertBizCode(submitAssemble(generateNumber("ZZD"), "0", depotId, sameMaterial),
                ExceptionConstants.DEPOT_HEAD_ASSEMBLE_SAME_MATERIAL_CODE);

        assertBizCode(submitAssemble(generateNumber("ZZD"), "0", 999999L, normalRows(false)),
                ExceptionConstants.DEPOT_DATA_PERMISSION_CODE);

        JSONArray overStock = normalRows(false);
        overStock.getJSONObject(1).put("operNumber", 999999);
        String overStockNumber = generateNumber("ZZD");
        Response overStockResponse = submitAssemble(overStockNumber, "0", depotId, overStock);
        JSONObject overStockBody = JSONObject.parseObject(overStockResponse.body().asString());
        if (overStockBody.getIntValue("code") == 200) {
            // 强审核模式允许先保存草稿，但审核时仍必须阻止子件库存变成负数。
    ...
    void enforceButtonAndDataPermission() {
        Response sample = authReqGet().param("id", 410).get(CONTEXT + "/depotHead/info");
        ...
        Response infoResponse = RestAssured.given().header("X-Access-Token", token)
                .param("id", 410).get(CONTEXT + "/depotHead/info");
        assertBizCode(infoResponse, ExceptionConstants.DEPOT_DATA_PERMISSION_CODE);

        JSONObject requestBody = buildRequest(generateNumber("ZZD"), "0", depotId, normalRows(false));
    ...
    private JSONArray normalRows(boolean sameCombinationAndComponent) {
        ...
        rows.add(buildRow(materials.get(0), "组合件", 2, depotId));
    ...
    private Response submitAssemble(String number, String status, Long rowDepotId, JSONArray rows) {
        for (int index = 0; index < rows.size(); index++) {
            rows.getJSONObject(index).put("depotId", rowDepotId);
        }
        return authReq().body(buildRequest(number, status, rowDepotId, rows).toJSONString())
                .post(CONTEXT + "/depotHead/addDepotHeadAndDetail");
    }

    private JSONObject buildRequest(String number, String status, Long rowDepotId, JSONArray rows) {
        JSONObject info = new JSONObject();
        info.put("number", number);
        info.put("type", "其它");
        info.put("subType", "组装单");
