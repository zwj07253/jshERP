...
CREATE TABLE IF NOT EXISTS jsh_ai_config (
    ...
    custom_prompt VARCHAR(4000),
    updated_by VARCHAR(100),
    updated_time TIMESTAMP
);

INSERT INTO jsh_ai_config (id, enabled, provider, timeout_seconds, max_file_mb, vision_enabled)
VALUES (1, FALSE, 'OpenAI Compatible', 60, 10, FALSE)
ON CONFLICT (id) DO NOTHING;

CREATE TABLE IF NOT EXISTS jsh_ai_import_log (
...
CREATE INDEX IF NOT EXISTS ix_ai_import_log_tenant_time
    ON jsh_ai_import_log(tenant_id, created_time DESC);

-- AI 配置只放在系统管理中，授予平台管理员和各租户的系统管理员角色。
INSERT INTO jsh_function (id, number, name, parent_number, url, component, state, sort, enabled, type, push_btn, icon, delete_flag)
VALUES (306, '000114', 'AI模型配置', '0001', '/system/ai_model_config', '/system/AiModelConfig', FALSE, '0176', TRUE, '电脑版', '1', 'robot', '0')
ON CONFLICT (id) DO UPDATE SET
number = EXCLUDED.number, name = EXCLUDED.name, parent_number = EXCLUDED.parent_number,
url = EXCLUDED.url, component = EXCLUDED.component, state = EXCLUDED.state, sort = EXCLUDED.sort,
enabled = EXCLUDED.enabled, type = EXCLUDED.type, push_btn = EXCLUDED.push_btn, icon = EXCLUDED.icon,
...
WHERE type = 'RoleFunctions'
  AND key_id IN (
      SELECT id::VARCHAR FROM jsh_role
      WHERE id = 4 OR name = '系统管理员'
  )
  AND value NOT LIKE '%[306]%';

UPDATE jsh_user_business
SET btn_str = CASE
    WHEN COALESCE(TRIM(btn_str), '') = '' THEN '[{"funId":306,"btnStr":"1"}]'
    ELSE LEFT(TRIM(btn_str), LENGTH(TRIM(btn_str)) - 1) || ',{"funId":306,"btnStr":"1"}]'
END
WHERE type = 'RoleFunctions'
  AND key_id IN (
      SELECT id::VARCHAR FROM jsh_role
  ...
  AND COALESCE(btn_str, '') NOT LIKE '%"funId":306%';
