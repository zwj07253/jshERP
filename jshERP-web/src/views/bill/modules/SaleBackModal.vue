<template>
  <j-modal
    ...
    <template slot="footer">
      ...
      <a-button v-if="billPrintFlag && isShowPrintBtn" @click="handlePrintPro('销售退货入库')">{{ $t('common.printNew') }}</a-button>
      <a-button v-if="billPrintFlag && isShowPrintBtn" @click="handlePrint('销售退货入库')">{{ $t('common.print') }}</a-button>
        ...
        <a-row class="form-row" :gutter="24">
          ...
          <a-col :lg="6" :md="12" :sm="24">
            <a-form-item :labelCol="labelCol" :wrapperCol="wrapperCol" :label="$t('common.billNo')">
              <a-input :placeholder="$t('common.enterBillNo')" v-decorator.trim="[ 'number', validatorRules.number ]" :readOnly="!!model.id" />
            </a-form-item>
          </a-col>
          <a-col :lg="6" :md="12" :sm="24">
            <a-form-item :labelCol="labelCol" :wrapperCol="wrapperCol" :label="$t('common.linkedBill')">
              <a-input-search :placeholder="$t('purchase.form.selectLinkedBill')" v-decorator="[ 'linkNumber' ]" @search="onSearchLinkNumber"
                              :readOnly="true" :disabled="!!model.id"/>
            </a-form-item>
          </a-col>
        </a-row>
        ...
        <a-row class="form-row" :gutter="24">
          ...
          <a-col :lg="6" :md="12" :sm="24">
            <a-form-item :labelCol="labelCol" :wrapperCol="wrapperCol" :label="$t('common.currentRefund')">
              <a-input :placeholder="$t('common.enterCurrentRefund')" v-decorator.trim="[ 'changeAmount' ]" @change="onChangeChangeAmount"/>
            </a-form-item>
          </a-col>
          <a-col :lg="6" :md="12" :sm="24">
            <a-form-item :labelCol="labelCol" :wrapperCol="wrapperCol" :label="$t('common.currentDebt')">
              <a-input :placeholder="$t('common.enterCurrentDebt')" v-decorator.trim="[ 'debt' ]" :readOnly="true"/>
            </a-form-item>
          </a-col>
          <a-col :lg="6" :md="12" :sm="24">
      ...
      return {
        ...
        materialTable: {
          ...
          columns: [
            ...
            { title: this.$t('common.originalQuantity'), key: 'preNumber', width: '5%', type: FormTypes.normal },
            { title: this.$t('common.returnedQuantity'), key: 'finishNumber', width: '5%', type: FormTypes.normal },
        ...
        validatorRules:{
          ...
          accountId:{
            rules: [
              { required: true, message: this.$t('purchase.validation.selectAccount') }
            ]
          }
        },
        url: {
    ...
    methods: {
      ...
      linkBillListOk(selectBillDetailRows, linkNumber, organId, discount, deposit, remark, depotId, accountId, salesMan) {
        ...
        if(selectBillDetailRows && selectBillDetailRows.length>0) {
          ...
          if(listEx.length === 0) {
            this.$nextTick(() => {
              this.$message.warning(this.$t('bill.allDetailsLinked'));
            })
          }
          ///给优惠后金额重新赋值
