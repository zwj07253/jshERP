<template>
  <j-modal
    ...
    <template slot="footer">
      ...
      <a-button v-if="billPrintFlag && isShowPrintBtn" @click="handlePrintPro('组装单')">{{ $t('common.printNew') }}</a-button>
      <a-button v-if="billPrintFlag && isShowPrintBtn" @click="handlePrint('组装单')">{{ $t('common.print') }}</a-button>
    ...
    data () {
      return {
        ...
        materialTable: {
          ...
          dataSource: [],
          columns: [
            { title: this.$t('common.productType'),key: 'mType',width:'7%', type: FormTypes.normal },
            ...
            { title: this.$t('common.quantity'), key: 'operNumber', width: '5%', type: FormTypes.inputNumber,
              validateRules: [{ required: true, message: '${title}不能为空' }]
            },
            { title: this.$t('purchase.form.columns.unitPrice'), key: 'unitPrice', width: '5%', type: FormTypes.inputNumber },
            { title: this.$t('common.amount'), key: 'allPrice', width: '5%', type: FormTypes.inputNumber },
