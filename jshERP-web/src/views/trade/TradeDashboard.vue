<template>
  <div class="trade-dashboard">
    <a-alert
      banner
      message="外贸手表 Demo：中国产购 → 国际发运 → 墨西哥清关 → 分销销售 → 到岸成本与毛利"
      type="info"
      show-icon
    ...
    <a-row :gutter="16">
      <a-col v-for="item in cards" :key="item.key" :xs="24" :sm="12" :xl="6" style="margin-bottom: 16px">
        <a-card :loading="loading" :bordered="false" class="metric-card">
          ...
          <div class="metric-value">¥{{ formatMoney(overview[item.key]) }}</div>
    ...
    <a-row :gutter="16">
      ...
      <a-col :xl="10" :lg="24" style="margin-bottom: 16px">
        <a-card title="清关与单证提醒" :loading="loading" :bordered="false" :body-style="{ padding: '0' }">
    ...
    <a-row :gutter="16">
      ...
      <a-col :xl="12" :lg="24" style="margin-bottom: 16px">
        <a-card title="已入库批次 SKU 到岸成本" :loading="loading" :bordered="false">
          <a-table :columns="profitColumns" :dataSource="profit" :pagination="false" rowKey="materialName" size="small">
            <span slot="landedCost" slot-scope="text">¥{{ formatMoney(text) }}</span>
            <span slot="estimatedGrossProfit" slot-scope="text">¥{{ formatMoney(text) }}</span>
          </a-table>
        </a-card>
      </a-col>
    </a-row>
  ...
  data () {
    return {
      ...
      cards: [
        ...
        { key: 'salesAmount', title: '销售出库金额', note: '当前手表销售单据' },
        { key: 'inTransitAmount', title: '海运在途货值', note: '已开船、尚未到港' },
        { key: 'customsAmount', title: '清关中货值', note: '到港待放行货物' },
        { key: 'availableStockAmount', title: '墨西哥可销售库存', note: '已入库且可分销' },
        { key: 'lockedStockAmount', title: '客户锁定库存', note: '已分配给客户订单' },
        { key: 'landedCost', title: '已核算到岸成本', note: '采购、物流、关税和本地费用' },
        { key: 'grossProfit', title: '演示毛利', note: '毛利率 ' }
      ],
      shipmentColumns: [
        { title: '批次号', dataIndex: 'shipmentNo', width: 150 },
        { title: '柜号', dataIndex: 'containerNo', width: 135 },
        { title: '线路', customRender: (text, record) => `${record.originPort} → ${record.destinationPort}` },
      ...
      profitColumns: [
        ...
        { title: '到岸成本', dataIndex: 'landedCost', scopedSlots: { customRender: 'landedCost' }, align: 'right' },
        { title: '预估毛利', dataIndex: 'estimatedGrossProfit', scopedSlots: { customRender: 'estimatedGrossProfit' }, align: 'right' }
      ]
    }
  },
  created () {
  ...
  methods: {
    async loadData () {
      ...
      try {
        ...
        this.profit = result[4].data.data || []
      } finally {
        this.loading = false
      }
    },
    ...
    statusColor (status) {
      return { '已入库': 'green', '清关中': 'orange', '在途': 'blue', '异常': 'red' }[status] || 'default'
    }
  }
}
</script>
