    ...
    <a-col :md="24">
      <a-card :style="cardStyle" :bordered="false">
        ...
        <div class="table-page-search-wrapper">
          ...
          <a-form layout="inline" @keyup.enter.native="searchQuery">
            ...
            <template v-if="toggleSearchStatus">
              <a-row :gutter="24">
                <a-col :md="6" :sm="24">
                <a-form-item :label="$t('retail.memberCard')" :labelCol="labelCol" :wrapperCol="wrapperCol">
                  <a-select :placeholder="$t('retail.selectMemberCard')" showSearch allow-clear optionFilterProp="children" v-model="queryParam.organId" @search="handleSearchRetail">
                ...
                <a-col :md="6" :sm="24">
                <a-form-item :label="$t('retail.operator')" :labelCol="labelCol" :wrapperCol="wrapperCol">
                  <a-select :placeholder="$t('retail.selectOperator')" showSearch allow-clear optionFilterProp="children" v-model="queryParam.creator">
                ...
                <a-col :md="6" :sm="24">
                <a-form-item :label="$t('retail.billStatus')" :labelCol="labelCol" :wrapperCol="wrapperCol">
                  <a-select :placeholder="$t('retail.selectStatus')" allow-clear v-model="queryParam.status">
                ...
                <a-col :md="6" :sm="24">
                <a-form-item :label="$t('retail.billRemark')" :labelCol="labelCol" :wrapperCol="wrapperCol">
                  <a-input :placeholder="$t('retail.enterRemark')" v-model="queryParam.remark"></a-input>
                  </a-form-item>
                </a-col>
              </a-row>
            </template>
        ...
        <div class="table-operator"  style="margin-top: 5px">
          ...
          <a-button v-if="quickBtn.retailBack.indexOf(1)>-1 && btnEnableList.indexOf(1)>-1" icon="share-alt" @click="transferBill($t('retail.transferToReturn'), quickBtn.retailBack)">{{ $t('retail.transferToReturn') }}</a-button>
          <a-button v-if="checkFlag && btnEnableList.indexOf(2)>-1" icon="check" @click="batchSetStatus(1)">{{ $t('retail.audit') }}</a-button>
          <a-button v-if="checkFlag && btnEnableList.indexOf(7)>-1" icon="stop" @click="batchSetStatus(0)">{{ $t('retail.unaudit') }}</a-button>
          <a-button v-if="isShowExcel && btnEnableList.indexOf(3)>-1" icon="download" @click="handleExport">{{ $t('retail.export') }}</a-button>
          <a-popover trigger="click" placement="right">
            ...
            <a-button icon="setting">{{ $t('common.columnSettings') }}</a-button>
                  ...
                  <a-tooltip placement="left" :title="$t('retail.retailOutTip')" slot="action">
        ...
        <div>
          <a-table
            ...
            <span slot="action" slot-scope="text, record">
              <a @click="myHandleDetail(record, '零售出库', prefixNo)">{{ $t('retail.view') }}</a>
              <a-divider v-if="btnEnableList.indexOf(1)>-1" type="vertical" />
              <a v-if="btnEnableList.indexOf(1)>-1" @click="myHandleEdit(record)">{{ $t('retail.edit') }}</a>
              <a-divider v-if="btnEnableList.indexOf(1)>-1" type="vertical" />
              <a v-if="btnEnableList.indexOf(1)>-1" @click="myHandleCopyAdd(record)">{{ $t('retail.copy') }}</a>
              <a-divider v-if="btnEnableList.indexOf(1)>-1" type="vertical" />
              <a-popconfirm v-if="btnEnableList.indexOf(1)>-1" :title="$t('retail.confirmDelete')" @confirm="() => myHandleDelete(record)">
                <a>{{ $t('retail.delete') }}</a>
              </a-popconfirm>
            </span>
  ...
  export default {
    ...
    data () {
      return {
        ...
        defColumns: [
          {
            title: this.$t('retail.action'),
            dataIndex: 'action',
            align:"center", width: 180,
            scopedSlots: { customRender: 'action' },
          },
          { title: this.$t('retail.member'), dataIndex: 'organName',width:120, ellipsis:true},
          ...
          { title: this.$t('retail.paymentType'), dataIndex: 'payType',width:80},
          { title: this.$t('retail.operator'), dataIndex: 'userName',width:80, ellipsis:true},
          { title: this.$t('retail.quantity'), dataIndex: 'materialCount',width:60},
          { title: this.$t('retail.totalAmount'), dataIndex: 'totalPrice',width:80},
          { title: this.$t('retail.receivedAmount'), dataIndex: 'getAmount',width:80,
            customRender:function (text,record,index) {
          ...
          { title: this.$t('retail.change'), dataIndex: 'backAmount',width:50},
          { title: this.$t('retail.receivingAccount'), dataIndex: 'accountName',width:80},
          { title: this.$t('retail.remark'), dataIndex: 'remark',width:200},
          { title: this.$t('retail.status'), dataIndex: 'status', width: 80, align: "center",
